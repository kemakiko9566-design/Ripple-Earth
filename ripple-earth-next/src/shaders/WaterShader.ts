...
export interface WaterShaderUniforms {
  ...
  uSunDir: Uniform<{ x: number; y: number; z: number }>;
...
vec3 gerstnerWave(vec4 wave, vec3 pos, inout vec3 tangent, inout vec3 binormal) {
  ...
  float freq = 6.2832 / wavelength;
  float phase = speed * freq;
  float theta = freq * dot(direction, pos.xz) + uTime * phase;
  float S = sin(theta);
  float C = cos(theta);
  float a = steepness / freq;
  vec3 displacement = vec3(direction.x * a * C, a * S, direction.y * a * C);
  vec3 wt = vec3(1.0 - direction.x * direction.x * steepness * S, direction.x * freq * a * C, -direction.x * direction.y * steepness * S);
  vec3 wb = vec3(-direction.x * direction.y * steepness * S, direction.y * freq * a * C, 1.0 - direction.y * direction.y * steepness * S);
  tangent += wt;
  binormal += wb;
  return displacement;
}
...
void main() {
  ...
  for (int i = 0; i < 8; i++) {
    ...
    vec4 wave = vec4(wl, uWaveHeight / fi, 0.3 + fi * 0.05, angle);
...
void main() {
  ...
  float foam = smoothstep(0.3, 0.7, fresnel) * smoothstep(-0.1, 0.2, vHeight);
